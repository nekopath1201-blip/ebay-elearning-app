...
キャラクター「チビ太」が案内するeBay業務研修用のEラーニングシステム。管理者がセクション・課題を作成し、受講者は順にこなして進捗を積み上げる。

同ディレクトリ内の「にゃん助Eラーニング」(elearning-app) を元にした、eBay研修専用の別プロジェクト。

## 課題に添付できるもの

課題タイプの区別はなく、1つの課題にテキスト・動画・画像・ファイルを自由に組み合わせて添付できる（すべて任意項目）。
...
動画・画像・ファイルの添付は、課題作成後の編集画面（`/admin/sections/[id]/tasks/[taskId]`）から行う。
