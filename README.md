...
猫のキャラクター「にゃん助」が案内するeBay業務研修用のEラーニングシステム。管理者がセクション・課題（動画/テキスト/ファイル/クイズ）を作成し、受講者は順にこなして進捗を積み上げる。
...
`src/lib/catMessages.ts` にイベント種別（開始/正解/不正解/課題完了/セクション完了/全完了）ごとの定型セリフを定義し、`getCatMessage()` でランダムに選択して表示する（AI呼び出しなし、ルールベース）。
